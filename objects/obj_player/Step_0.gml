...
{
	if global.cloth >= 5 && global.wood >= 5
	{
		if instance_exists(obj_wind_gust)
		{
			if obj_wind_gust.blowing_hard && obj_wind_gust.can_tent && !instance_exists(obj_player_tent)
			{
				...
				instance_create_layer(x,y,"Main",obj_player_tent)
			}
		}
	}
	else
	{
		play_sound(0,snd_cant_craft,0,0.3,0.95,0.1,250,400,1,80)
		red_alpha[1]=1.2
		red_alpha[3]=1.2
	}
}
...
{
	if global.rocks >= 5 && global.wood >= 5
	{
		play_sound(0,snd_craft,0,0.3,0.95,0.1,250,400,1,80)
...
{
	if global.cloth >= 5 && global.rocks >= 5 && player_hp < 100
	{
		play_sound(0,snd_craft,0,0.3,0.95,0.1,250,400,1,80)
	...
	{
		if player_hp >= 100 {play_sound(0,snd_cant_craft,0,0.3,0.95,0.1,250,400,1,80)}
		else
		{
			play_sound(0,snd_cant_craft,0,0.3,0.95,0.1,250,400,1,80)
			red_alpha[2]=1.2
...
if player_hp <= 0
{
	play_sound(1,snd_player_death,0,0.15,0.95,0.1,250,400,1,80)
	play_sound(1,snd_bass_death,0,0.06,1,0,250,400,1,80)
	
	instance_create_layer(x,y+10,"Main",obj_player_death)
	
	with (obj_chop_mask)   {instance_destroy()}
	with (obj_player_mask) {instance_destroy()}
...
if x > 7200
{
	if !succeeded
	{
		//play_sound(0,snd_success,0,0.1,1,0,250,400,1,80)
...
if angle=0   {shake_amt=0}
if angle=355 {shake_amt=0.2}
if angle=350 {shake_amt=0.4}

if instance_exists(obj_wind_gust)
{
	if obj_wind_gust.blowing_hard
	{
		red_flash=wave(0,1,0.5,0)
	}
	else {red_flash=0}
} else {red_flash=0}
